...
gfortran -c -g -m64 -O3  -I"${MKLROOT}/include" saving.f90  -o saving.o
gfortran -c -g -m64 -O3  -I"${MKLROOT}/include" tdEvolve.f90   -o tdEvolve.o
gfortran -c -g -m64 -O3  -I"${MKLROOT}/include" main.f90   -o main.o
if [[ $OPENMP -eq 0 ]]; then
    LINKLINE="-Wl,--start-group ${MKLROOT}/lib/intel64/libmkl_gf_lp64.a ${MKLROOT}/lib/intel64/libmkl_sequential.a ${MKLROOT}/lib/intel64/libmkl_core.a -Wl,--end-group -lpthread -lm -ldl"
else
    LINKLINE=" -Wl,--start-group ${MKLROOT}/lib/intel64/libmkl_gf_lp64.a ${MKLROOT}/lib/intel64/libmkl_gnu_thread.a ${MKLROOT}/lib/intel64/libmkl_core.a -Wl,--end-group -lgomp -lpthread -lm -ldl"
fi
gfortran -g -m64   main.o tdEvolve.o bases.o params.o saving.o modSlaterIndex.o operators.o $LINKLINE -o main #-L${MKLROOT}/lib/intel64  -lblas -llapack
